...
\begin{figure}[hbt!]
    ...
    \caption{Time Per Query (s) vs Query Index - United States.}
\end{figure}

\begin{table}[ht]
    \caption{\label{tab:table1} Time Data for Each of the Methods - US Testcase}
    \centering
    \begin{tabular}{lccc}
    \hline
    \textbf{Metric} & \textbf{EPQS} & \textbf{DEM} & \textbf{Google LLC} \\
    \hline
...
Figure 1 shows the time taken per query of latitude and longitude pairs in each of the 3 methods for queries generated in the Continental United States. Due to scaling factors, it can be noted from the associated minimum, average and maximum times, the difference between each method. It is clear that the DEM methods, even with it's delay due to indexing is consistent and able to complete a query in less than a 100\textsuperscript{th} of a second.
